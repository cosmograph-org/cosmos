...
export class Graph {
  ...
  private canvasD3Selection: Selection<
    HTMLCanvasElement,
    undefined,
    null,
    undefined
  ...
  public constructor(canvas: HTMLCanvasElement, config?: GraphConfigInterface) {
    ...
    if (this.config.disableZoom || !this.config.enableDrag)
    ...
    this.store.maxPointSize =
      (this.device.limits.pointSizeDims[1] ?? MAX_POINT_SIZE) /
      this.config.pixelRatio;
    this.store.adjustSpaceSize(
      this.config.spaceSize,
      this.device.limits.maxTextureSize
    );
  ...
  public setConfig(config: Partial<GraphConfigInterface>): void {
    ...
    if (
      prevConfig.curvedLinkSegments !== this.config.curvedLinkSegments ||
    ...
    if (prevConfig.backgroundColor !== this.config.backgroundColor)
      this.store.backgroundColor = getRgbaColor(this.config.backgroundColor);
    if (
      prevConfig.hoveredPointRingColor !== this.config.hoveredPointRingColor
    ) {
      this.store.setHoveredPointRingColor(this.config.hoveredPointRingColor);
    }
    if (
      prevConfig.focusedPointRingColor !== this.config.focusedPointRingColor
    ) {
      this.store.setFocusedPointRingColor(this.config.focusedPointRingColor);
    }
    ...
    if (
      prevConfig.spaceSize !== this.config.spaceSize ||
      prevConfig.simulation.repulsionQuadtreeLevels !==
    ...
    if (
      prevConfig.disableZoom !== this.config.disableZoom ||
   ...
   */
  public zoomToPointByIndex(
    index: number,
    duration = 700,
    scale = defaultScaleToZoom,
    canZoomOut = true
  ): void {
    const {
      store: { screenSize },
    } = this;
    const positionPixels = readPixels(
      this.device,
      this.points.currentPositionFbo as regl.Framebuffer2D
    );
  ...
  public getPointPositions(): number[] {
    ...
    const pointPositionsPixels = readPixels(
      this.device,
      this.points.currentPositionFbo as regl.Framebuffer2D
    );
   ...
   * @param padding Padding around the viewport in percentage
   */
  public fitViewByPointPositions(
    positions: number[],
    duration = 250,
    padding = 0.1
  ): void {
  ...
  public getPointsInRange(
    ...
    const pixels = readPixels(
      this.device,
      this.points.selectedFbo as regl.Framebuffer2D
    );
  ...
  public create(): void {
    ...
    if (this._hasLinksChanged || this._hasPointPositionsChanged)
    ...
    if (this._hasPointClustersChanged || this._hasClusterPositionsChanged)
  ...
  public pair(pointPositions: number[]): [number, number][] {
    ...
    for (let i = 0; i < pointPositions.length / 2; i++) {
      arr[i] = [
        pointPositions[i * 2] as number,
        pointPositions[i * 2 + 1] as number,
      ];
    }
  ...
  private update(simulationAlpha = this.store.alpha): void {
    const { graph } = this;
    this.store.pointsTextureSize = Math.ceil(
      Math.sqrt(graph.pointsNumber ?? 0)
    );
    this.store.linksTextureSize = Math.ceil(
      Math.sqrt((graph.linksNumber ?? 0) * 2)
    );
    this.create();
    this.initPrograms();
  ...
  private frame(): void {
    const {
      config: { simulation, renderLinks, disableSimulation },
      store: { alpha, isSimulationRunning },
    } = this;
      ...
      if (!disableSimulation) {
        ...
        if (isSimulationRunning && !this.zoomInstance.isRunning) {
          ...
          this.store.alpha += this.store.addAlpha(
            this.config.simulation.decay ?? defaultConfigValues.simulation.decay
          );
          if (this.isRightClickMouse)
            this.store.alpha = Math.max(this.store.alpha, 0.1);
          this.store.simulationProgress = Math.sqrt(
            Math.min(1, ALPHA_MIN / this.store.alpha)
          );
  ...
  private updateMousePosition(
    ...
    const mouseX =
      ...
      (event as D3DragEvent<HTMLCanvasElement, undefined, Hovered>).x;
    const mouseY =
      (event as MouseEvent).offsetY ??
      (event as D3DragEvent<HTMLCanvasElement, undefined, Hovered>).y;
  ...
  private resizeCanvas(forceResize = false): void {
    ...
    if (
      forceResize ||
      prevWidth !== w * this.config.pixelRatio ||
      prevHeight !== h * this.config.pixelRatio
    ) {
      const [prevW, prevH] = this.store.screenSize;
      const { k } = this.zoomInstance.eventTransform;
      const centerPosition = this.zoomInstance.convertScreenToSpacePosition([
        prevW / 2,
        prevH / 2,
      ]);
      ...
      this.canvasD3Selection.call(
        ...
        this.zoomInstance.getTransform([centerPosition], k)
      );
  ...
  private updateZoomDragBehaviors(): void {
    if (this.config.enableDrag) {
      ...
      this.canvasD3Selection.call(this.dragInstance.behavior).on(".drag", null);
    }
  ...
  private findHoveredPoint(): void {
    ...
    const pixels = readPixels(
      this.device,
      this.points.hoveredFbo as regl.Framebuffer2D
    );
  ...
  private updateCanvasCursor(): void {
    ...
    if (this.dragInstance.isActive)
      select(this.canvas).style("cursor", "grabbing");
    else if (this.store.hoveredPoint) {
      if (!this.config.enableDrag || this.store.isSpaceKeyPressed)
        select(this.canvas).style("cursor", hoveredPointCursor);
